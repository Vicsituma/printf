		...
		int flags, int width, int precision, int size)
{
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS) /* Asign extrs char to left of buffer [buffer>padd]*/
		{
 ...
 * Return: Number ofwritten chars.
 */
int write_pointer(char buffer[], int ind, int length, int width,
		int flags, int padd, int extra_c, int padd_start)
{
	int i;
